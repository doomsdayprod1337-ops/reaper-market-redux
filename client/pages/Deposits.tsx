    ...
    return (
      <div className="min-h-screen bg-reaper-black flex items-center justify-center relative overflow-hidden">
        ...
        <div className="relative z-10 text-white text-center">
          <h1 className="text-2xl font-bold mb-4 font-reaper text-reaper-red text-glow-red">Access Denied</h1>
      ...
      <div className="relative z-10 text-white">
        <div className="container mx-auto px-4 py-8">
          <div className="flex justify-between items-center mb-8 animate-in slide-in-from-top duration-500">
            <div>
              <h1 className="text-3xl font-bold font-reaper text-reaper-red text-glow-red animate-glow">💰 Crypto Deposits</h1>
              <p className="text-gray-400 text-sm mt-1">Minimum deposit amount: ${minimumDepositAmount?.toFixed(2) || '50.00'}</p>
            </div>
            <button
              onClick={() => setShowCreateForm(!showCreateForm)}
              className="bg-reaper-red hover:bg-reaper-red-light px-6 py-3 rounded-lg font-medium transition-all duration-300 border-glow-red hover:scale-[1.02]"
            >
          ...
          {error && (
            <div className="bg-red-900/50 border border-red-500 text-white p-4 rounded-lg mb-6 animate-in slide-in-from-top duration-300 backdrop-blur-sm">
              {error}
            </div>
          )}
          {success && (
            <div className="bg-reaper-red/20 border border-reaper-red text-reaper-red-light p-4 rounded-lg mb-6 animate-in slide-in-from-top duration-300 backdrop-blur-sm border-glow-red">
              {success}
            </div>
          )}
          ...
          {preSelectedCurrency && preSelectedAmount && (
            <div className="bg-reaper-red/20 border border-reaper-red text-white p-4 rounded-lg mb-6 animate-in slide-in-from-left duration-500 backdrop-blur-sm">
          ...
          {/* Create Deposit Form */}
          {showCreateForm && (
            <div className="bg-reaper-dark-gray/80 rounded-lg p-6 mb-8 border border-reaper-red/30 backdrop-blur-sm animate-in slide-in-from-right duration-700">
              <h2 className="text-xl font-semibold mb-4 font-reaper text-reaper-red">Create New Deposit</h2>
              <form onSubmit={handleCreateDeposit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 bg-reaper-medium-gray border border-reaper-red/30 rounded text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-reaper-red focus:border-reaper-red transition-all duration-300"
                      required
                    />
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={handleInputChange}
                      className="w-full px-3 py-2 bg-reaper-medium-gray border border-reaper-red/30 rounded text-white focus:outline-none focus:ring-2 focus:ring-reaper-red"
                    >
                ...
                {formData.currency && walletSettings?.currencies?.[formData.currency] && (
                  <div className="bg-reaper-medium-gray/50 rounded-lg p-4 border border-reaper-red/20">
                    <h3 className="font-medium mb-2 text-reaper-red">Currency Limits</h3>
                ...
                <div className="flex justify-end space-x-4">
                  <button
                    ...
                    className="px-6 py-2 bg-reaper-medium-gray hover:bg-reaper-dark-gray rounded-lg transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="px-6 py-2 bg-reaper-red hover:bg-reaper-red-light disabled:bg-gray-600 disabled:cursor-not-allowed rounded-lg transition-all duration-300 border-glow-red"
                  >
          ...
          {/* Deposits List */}
          <div className="bg-reaper-dark-gray/80 rounded-lg p-6 border border-reaper-red/30 backdrop-blur-sm animate-in slide-in-from-bottom duration-700">
            <h2 className="text-xl font-semibold mb-4 font-reaper text-white">Your Deposits</h2>
            
            {loading ? (
              <div className="text-center py-8">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-reaper-red mx-auto"></div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-reaper-red/20">
                      <th className="text-left py-3 px-4 font-reaper text-gray-300">Currency</th>
                      <th className="text-left py-3 px-4 font-reaper text-gray-300">Amount</th>
                      <th className="text-left py-3 px-4 font-reaper text-gray-300">Status</th>
                      <th className="text-left py-3 px-4 font-reaper text-gray-300">Wallet Address</th>
                      <th className="text-left py-3 px-4 font-reaper text-gray-300">Created</th>
                      <th className="text-left py-3 px-4 font-reaper text-gray-300">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {deposits.map((deposit) => (
                      <tr key={deposit.id} className="border-b border-reaper-red/10 hover:bg-reaper-medium-gray/20 transition-colors">
                        ...
                        <td className="py-3 px-4">
                          <div className="max-w-xs">
                            <code className="text-xs bg-reaper-medium-gray px-2 py-1 rounded break-all">
                        ...
                        <td className="py-3 px-4">
                          {deposit.status === 'pending' && (
                            <button
                              ...
                              className="text-reaper-red hover:text-reaper-red-light text-sm transition-colors"
                            >
                              Copy Address
                            </button>
                          )}
          ...
          {/* Instructions */}
          <div className="bg-reaper-red/10 border border-reaper-red/30 rounded-lg p-6 mt-8 backdrop-blur-sm border-glow-red animate-in slide-in-from-bottom duration-700 delay-200">
            <h3 className="text-lg font-semibold text-reaper-red mb-3 font-reaper">How to Deposit</h3>
